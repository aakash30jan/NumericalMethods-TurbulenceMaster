...
pl.plot(np.log10(data1[:,0]),np.log10(data1[:,1]),'r',label='slope='+str(slope-1))
pl.legend()
...
pl.title('Error in computation of tridiagonal matrix (n=4 to 30660)')
pl.loglog(data1[:,0],data1[:,1],'g',label='slope='+str(slope-1))
pl.legend()
...
pl.plot(np.log10(data2[:,0]),np.log10(data2[:,1]),'r',label='slope='+str(slope-1))
pl.legend()
...
pl.ylabel('Mean Error')
pl.title('Error in computation of tridiagonal matrix (n=4 to 10000 with step of 10)')
pl.loglog(data2[:,0],data2[:,1],'g',label='slope='+str(slope-1))
pl.legend()
